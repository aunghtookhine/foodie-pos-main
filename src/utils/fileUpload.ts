...
export const fileUpload = multer({
  storage: multerS3({
    ...
    acl: "public-read",
    key: (request, file, cb) => {
      cb(null, `foodie-pos/aung-htoo-khine/${Date.now()}_${file.originalname}`);
    },
  }),
}).array("files", 1);
...
export const getQrCodeUrl = (tableId: number) => {
  return `https://msquarefdc.sgp1.cdn.digitaloceanspaces.com/foodie-pos/aung-htoo-khine/qrcode/tableId-${tableId}.png`;
};
...
export const qrCodeImageUpload = async (tableId: number) => {
  try {
    ...
    const input = {
      Bucket: "msquarefdc",
      Key: `foodie-pos/aung-htoo-khine/qrcode/tableId-${tableId}.png`,
      ACL: "public-read",
      Body: Buffer.from(
